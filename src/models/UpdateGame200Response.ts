/* tslint:disable */
/* eslint-disable */
/**
 * MetaFab API
 * Complete MetaFab API references and guides can be found at: https://trymetafab.com
 *
 * The version of the OpenAPI document: 1.5.1
 * Contact: metafabproject@gmail.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface UpdateGame200Response
 */
export interface UpdateGame200Response {
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    id?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    walletId?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    fundingWalletId?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    email?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    name?: string;
    /**
     * This field has not had a description added.
     * @type {object}
     * @memberof UpdateGame200Response
     */
    rpcs?: object;
    /**
     * This field has not had a description added.
     * @type {object}
     * @memberof UpdateGame200Response
     */
    redirectUris?: object;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    iconImageUrl?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    coverImageUrl?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    primaryColorHex?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    publishedKey?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    secretKey?: string;
    /**
     * This field has not had a description added.
     * @type {boolean}
     * @memberof UpdateGame200Response
     */
    verified?: boolean;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    updatedAt?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    createdAt?: string;
    /**
     * This field has not had a description added.
     * @type {string}
     * @memberof UpdateGame200Response
     */
    walletDecryptKey?: string;
}

/**
 * Check if a given object implements the UpdateGame200Response interface.
 */
export function instanceOfUpdateGame200Response(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function UpdateGame200ResponseFromJSON(json: any): UpdateGame200Response {
    return UpdateGame200ResponseFromJSONTyped(json, false);
}

export function UpdateGame200ResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): UpdateGame200Response {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'id': !exists(json, 'id') ? undefined : json['id'],
        'walletId': !exists(json, 'walletId') ? undefined : json['walletId'],
        'fundingWalletId': !exists(json, 'fundingWalletId') ? undefined : json['fundingWalletId'],
        'email': !exists(json, 'email') ? undefined : json['email'],
        'name': !exists(json, 'name') ? undefined : json['name'],
        'rpcs': !exists(json, 'rpcs') ? undefined : json['rpcs'],
        'redirectUris': !exists(json, 'redirectUris') ? undefined : json['redirectUris'],
        'iconImageUrl': !exists(json, 'iconImageUrl') ? undefined : json['iconImageUrl'],
        'coverImageUrl': !exists(json, 'coverImageUrl') ? undefined : json['coverImageUrl'],
        'primaryColorHex': !exists(json, 'primaryColorHex') ? undefined : json['primaryColorHex'],
        'publishedKey': !exists(json, 'publishedKey') ? undefined : json['publishedKey'],
        'secretKey': !exists(json, 'secretKey') ? undefined : json['secretKey'],
        'verified': !exists(json, 'verified') ? undefined : json['verified'],
        'updatedAt': !exists(json, 'updatedAt') ? undefined : json['updatedAt'],
        'createdAt': !exists(json, 'createdAt') ? undefined : json['createdAt'],
        'walletDecryptKey': !exists(json, 'walletDecryptKey') ? undefined : json['walletDecryptKey'],
    };
}

export function UpdateGame200ResponseToJSON(value?: UpdateGame200Response | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'id': value.id,
        'walletId': value.walletId,
        'fundingWalletId': value.fundingWalletId,
        'email': value.email,
        'name': value.name,
        'rpcs': value.rpcs,
        'redirectUris': value.redirectUris,
        'iconImageUrl': value.iconImageUrl,
        'coverImageUrl': value.coverImageUrl,
        'primaryColorHex': value.primaryColorHex,
        'publishedKey': value.publishedKey,
        'secretKey': value.secretKey,
        'verified': value.verified,
        'updatedAt': value.updatedAt,
        'createdAt': value.createdAt,
        'walletDecryptKey': value.walletDecryptKey,
    };
}

